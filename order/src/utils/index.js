...
module.exports.GenerateSignature = async (payload) => {
  return await jwt.sign(payload, APP_SECRET, { expiresIn: "31d" });
}
...
module.exports.CreateChannel = async () => {
  try {
    ...
    console.log('Channel created with exchange name', EXCHANGE_NAME, 'and type', 'direct');
    return channel;
  } catch (error) {
    throw error;
  }
...
module.exports.PublishMessage = async (channel, bindingKey, message) => {
  try {
    ...
    console.log("Message sent from order service!");
  } catch (error) {
    throw error;
  }
};
...
module.exports.SubscribeToMessage = async (channel, service) => {
  try {
    ...
    await channel.bindQueue(appQueue.queue, EXCHANGE_NAME, ORDER_BINDING_KEY);
    ...
    console.log('subscribed to exchange name', EXCHANGE_NAME)
  } catch (error) {
    throw error;
  }
};
